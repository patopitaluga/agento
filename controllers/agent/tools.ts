 ...
 *
 * At startup, built-in file tools are created for `WORKSPACE_DIR`
 * (`resolveWorkspaceDir`), filtered by `TOOL_PROFILE` (`getToolProfile`) and
 * `DISABLED_TOOLS` (`getDisabledTools`), then merged with tools from
 * local plugins in the gitignored `plugins/` folder (`loadPluginTools`).
 * The combined list is passed to `TurnSessionManager` and drives both the
 * Realtime API tool registry and `buildAgentInstructions`.
 *
 * **Exports** (1 function):
 * - `loadAgentTools` — assembles built-ins + plugins, applies filters,
 ...
 *
 * Used in:
 * - `controllers/agent/tools.ts` — `loadAgentTools`
 */
